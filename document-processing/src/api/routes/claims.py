...
async def get_db_pool():
    ...
    if not hasattr(app.state, "pool") or app.state.pool.is_closed():
...
async def verify_token(
    ...
    try:
        ...
        if not exp_timestamp or datetime.fromtimestamp(exp_timestamp, UTC) < datetime.now(UTC):
...
@v1_router.post("/claims", response_model=ClaimResponse, 
               dependencies=[Depends(RateLimiter(times=5, seconds=60, key="user_id"))],
