...
async def process_batch(claims: list):
    ...
    success_count = 0
    for claim in claims:
        if await process_claim(claim):
            success_count += 1
    processing_time = time.time() - start_time
    logger.info(f"Processed {len(claims)} claims in {processing_time:.2f}s, success rate: {success_count / len(claims):.2%}")

import asyncio

async def process_batches(queue):
    while True:
